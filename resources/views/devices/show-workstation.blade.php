    ...
    <script>
        ...
        commandForm?.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (!response.ok) {
                    addSerialLog(`Error ${response.status}: ${data.message || data.error || data.errorText || 'Failed to send command'}`, 'user');
                    return;
                }
                ...
                commandHistory.unshift({
                    id: data.command?.id || data.command_id,
                    command,
                    status: 'pending',
                    timestamp: new Date()
                });
        ...
        async function loadHistoricalLogs() {
            try {
                ...
                const data = await parseJsonResponse(response);
                if (!response.ok) {
                    console.error('Failed to load historical logs:', data.message || data.error || response.statusText);
                    return;
                }
                    ...
                    logPatterns = data.patterns.map(pattern => ({
                        ...
                        format: (matches) => {
                            ...
                            if (pattern.parser_config?.extractor === 'key_value_pairs') {
                                const d = matches[1] || matches[0];
                                const kvRegex = /(\w+)=([\d.]+)/g;
                                const parsed = {};
                                let match;
                                while ((match = kvRegex.exec(d)) !== null) {
